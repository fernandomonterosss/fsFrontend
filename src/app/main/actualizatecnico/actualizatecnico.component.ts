...
@Component({
  selector: 'actualizatecnico',
  templateUrl: './actualizatecnico.component.html',
  styleUrls: ['./actualizatecnico.component.scss']
})

export class ActualizatecnicoComponent implements  OnInit, AfterViewInit {
  edited=false;
  buzon : Buzon;
  displayedColumnStatus = [];
  displayedColumns = ['mantenimientos','id','code', 'lastName', 'email','telephonemovile', 'observaciones','fecha_recepcion','estado','servicio'];
  ...
  editView(orden,traslado){
    
    this.router.navigateByUrl('/ordenestecnico/'+orden+'/'+traslado)
  }
 ...
 resetForm(valor){
  this.dataSource.loadbuzon('','','','','',1,0,3);
   ...
   ngOnInit() {
    ...
    this.dataSource.loadbuzon('','','','','tecnicos','','asc',0,10,"id");
    this.dataSource.dataSource$.subscribe(result => {
...
export class BuzonDataSource implements DataSource<Buzon> {
  ...
  loadbuzon(nombre: string, apellido: string, orden: string, estado: string,user:string,filter: string,
             ...
             this.buzonService.getBuzon('3','c',nombre,apellido,orden,estado,user,filter,sortDirection,pageIndex, pageSize,sortfield)
              .pipe(
              catchError(() => of([])),
