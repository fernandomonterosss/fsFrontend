...
export class OrdenesComponent implements  OnInit, AfterViewInit {
    ...
    displayedColumns = ['editar','id', 'fecha_recepcion', 'responsable', 'estado','observaciones','mantenimientos'];
    ...
    SelectOrdenes(){
         if (this.varOrden==undefined) 
         this.varOrden=''
         if (this.varEstado==undefined)
         this.varEstado=''
         if (this.varResponsable==undefined)
         this.varResponsable=''
            this.dataSource.loadOrdenes(this.varOrden,this.varEstado,this.varResponsable,this.Id,'asc',this.paginator.pageIndex,this.paginator.pageSize);
         this.dataSource.dataSource$.subscribe(result => { 
          for (let r in result){
                 this.total=result[r];
            }
 ...
 btnOrden(url,id){
    var orden=0
    var nombre=' '
    var myurl = `${url}${id}/${this.nombre}/${this.Id}/0/0`;
    this.router.navigateByUrl(myurl)  
  }

  btnOrden2(url,id){

    var myurl = `${url}${id}/${this.nombre}/${this.Id}/2/0`;
    this.router.navigateByUrl(myurl)  
  }
